...
describe RedmineProjectWorkflows do
  ...
  it 'is patched into the host that booted this suite' do
    ...
    # (F01 of 2026-08-28-claude-audit). What it carries since ADR-003 is the
    # matrix cells: core's own workflows/_form draws the plugin's mixed cell and
    # its row and column actions, and core's controller is the only one of the
    # three that cannot name a helper in its own class body.
    # spec/controllers/workflows_helper_attachment_spec.rb is where the
