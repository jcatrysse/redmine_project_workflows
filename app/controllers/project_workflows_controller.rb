...
class ProjectWorkflowsController < ApplicationController
  ...
  # The drawing's role selection, which is the one selection on this controller
  # that is a *list*. Extracted for the reason the two above were: Metrics/
  # ClassLength crossed at 204/200, and that limit is already relaxed in
  # .rubocop.yml with a stated rationale -- so crossing it is a signal to
  # extract, not a cop to placate. Every method there is private: a public
  # instance method of a controller is an action.
  include RedmineProjectWorkflows::GraphSelection
