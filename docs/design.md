...
added later than the other three: the copy screen.
`ProjectWorkflowRulesController#duplicate` calls
...
| `WorkflowsController#index` | Redmine's own summary page | **copy** | replaced with `project_id: nil`, so a project's rules can never be added into the generic totals — which is exactly what core counted before any project had its own (claude F01) |
| `WorkflowsController#edit`, `#permissions` | Redmine's own two matrices | **copy** | replaced with `project_id: nil`, for the same reason |
| `WorkflowsController#find_statuses` | the "only used statuses" checkbox on those screens | **copy** | replaced with `project_id: nil`: core's own query would offer the generic matrix a status only some project's own workflow uses |
| `WorkflowsController#update`, `#update_permissions` | saving Redmine's own matrix | **left alone** | core's own bodies run. Their writes go through `WorkflowTransition.replace_transitions` / `WorkflowPermission.replace_permissions`, which the two rows below route to the plugin's writers with `project_id` fixed at `nil` — so the generic path is validated and isolated without a line here (INV-1) |
| `WorkflowsController#copy`, `#duplicate` | Redmine's own copy screen | **left alone** | core's own bodies run, and copy the generic workflow only: `WorkflowRule.copy` calls `.copy_one`, which the plugin scopes to `project_id: nil`. The plugin's own copy screen — six selectors, project targets, scope recording, and `invalid_copy_selection?` in front of all of it — is `ProjectWorkflowRulesController#duplicate` |
| `ProjectWorkflowRulesController` (all seven actions and four finders) | the plugin's own administration area | **copy** | the same bodies, with the project dimension: the selection may name projects, the generic workflow or both, and every query carries the resolved list (ADR-003). Watched by the drift gate exactly as a patch module is — a copy is a copy wherever it is filed |
...
| `WorkflowPermission.rules_by_status_id` | core's `permissions` action | **left alone** | project-blind, and unreachable once the plugin is installed because that action is replaced by one asking `PermissionQuery` for the generic population. Left alone; a plugin that calls it directly gets every project's rows |
